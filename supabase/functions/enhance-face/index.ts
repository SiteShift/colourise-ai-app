...
serve(async (req: Request) => {
  ...
  try {
    ...
    // 2. Check rate limit
    const allowed = await checkRateLimit(adminClient, user.id, 'enhance-face')
    if (!allowed) {
      return new Response(
    ...
    try {
      ...
      if (downloadError || !imageData) {
        // Try original-images bucket
        const { data: origData, error: origError } = await adminClient.storage
          .from('original-images')
          .download(storagePath)

        if (origError || !origData) {
          throw new Error(`Failed to download image from either bucket`)
        }
      }

      const imageBlob = imageData || (await adminClient.storage
        .from('original-images')
        .download(storagePath)).data!
      ...
      // 7. Upload to Cloudinary for face enhancement
      ...
      // 8. Apply face enhancement transformation
      const enhancedUrl = `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/image/upload/e_upscale,e_improve:face/f_auto,q_auto/${publicId}`

      // 9. Download enhanced image
      const enhancedResponse = await fetch(enhancedUrl)
      if (!enhancedResponse.ok) {
        throw new Error('Failed to fetch enhanced image from Cloudinary')
      }
      const enhancedBlob = await enhancedResponse.blob()

      // 10. Upload result to our Storage
      const resultPath = `${user.id}/${Date.now()}_face_enhanced.jpg`
      const { error: uploadError } = await adminClient.storage
        .from('colorized-images')
        .upload(resultPath, enhancedBlob, {
